jQuery(document).ready(function ($) {
  ...
  if ("plugins.php" == wps_onboarding.current_screen) {
    ...
    if (
      jQuery("#show-counter").length > 0 &&
      jQuery("#show-counter").val() == "not-sent"
    ) {
  ...
  jQuery(document).on("submit", "form.wps-on-boarding-form", function (e) {
    ...
    var form_data = JSON.stringify(
      jQuery("form.wps-on-boarding-form").serializeArray()
    );
  ...
  function wps_show_onboard_popup() {
    jQuery(".wps-onboarding-section-funnel-bulder").show();
    jQuery(".wps-on-boarding-wrapper-background").addClass(
      "onboard-popup-show"
    );
  ...
  function wps_hide_onboard_popup() {
    jQuery(".wps-on-boarding-wrapper-background").removeClass(
      "onboard-popup-show"
    );
    jQuery(".wps-onboarding-section-funnel-bulder").hide();
    if (!jQuery("body").hasClass("mobile-device")) {
  ...
  function add_deactivate_slugs_callback(all_slugs) {
    for (var i = all_slugs.length - 1; i >= 0; i--) {
      jQuery(document).on("click", "#deactivate-" + all_slugs[i], function (e) {
        ...
        var placeholder = jQuery("#deactivation-reason-text").attr(
          "placeholder"
        );
        jQuery("#deactivation-reason-text").attr(
          "placeholder",
