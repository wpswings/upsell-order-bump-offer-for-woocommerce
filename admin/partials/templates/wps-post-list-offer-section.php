				...
				<div class="wps-ubo-bump-card">
					<div class="wps-ubo-bump-card__header">
						<div class="wps-ubo-bump-card__title">
							...
							<?php endif; ?>
							<a class="wps_upsell_funnel_list_name" href="?page=upsell-order-bump-offer-for-woocommerce-setting&tab=creation-setting&sub_tab=post-list-offer-section&funnel_id=<?php echo esc_html( $key ); ?>"><?php echo esc_html( $funnel_name ); ?></a>
						</div>
					...
					<div class="wps-ubo-bump-card__body">
						...
						<div class="wps-ubo-bump-block">
							<div class="wps-ubo-block-title"><?php esc_html_e( 'Offers', 'upsell-order-bump-offer-for-woocommerce' ); ?></div>
							<?php
							if ( ! empty( $value['wps_wocuf_products_in_offer'] ) ) {
								...
								foreach ( $value['wps_wocuf_products_in_offer'] as $offer_key => $single_offer_product ) :
									...
									?>
									<p><?php echo '<strong>' . esc_html__( 'Offer', 'upsell-order-bump-offer-for-woocommerce' ) . ' #' . esc_html( $offer_key ) . '</strong> &rarr; ' . esc_html( $product->get_title() ) . '( #' . esc_html( $single_offer_product ) . ' )'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
									<?php
								endforeach;
								echo '</div>';
							} else {
						...
						<?php if ( 'yes' === $global_funnel || 'yes' === $exclusive_offer || 'yes' === $smart_offer_upgrade ) : ?>
							<div class="wps-ubo-bump-block">
								<div class="wps-ubo-block-title"><?php esc_html_e( 'Funnel Attributes', 'upsell-order-bump-offer-for-woocommerce' ); ?></div>
						...
						<div class="wps-ubo-bump-block wps-ubo-bump-actions">
							<a class="wps_wocuf_pro_funnel_links wps_upsell_bump_links" href="?page=upsell-order-bump-offer-for-woocommerce-setting&tab=creation-setting-post&sub_tab=post-list-offer-section&funnel_id=<?php echo esc_html( $key ); ?>&manage_nonce=<?php echo esc_html( wp_create_nonce( 'manage_funnel' ) ); ?>"><?php esc_html_e( 'View / Edit', 'upsell-order-bump-offer-for-woocommerce' ); ?></a>

							<a class="wps_wocuf_pro_funnel_links wps_upsell_bump_links" href="?page=upsell-order-bump-offer-for-woocommerce-setting&tab=one-click-section&sub_tab=post-list-offer-section&del_funnel_id=<?php echo esc_html( $key ); ?>&del_nonce=<?php echo esc_html( wp_create_nonce( 'del_funnel' ) ); ?>"><?php esc_html_e( 'Delete', 'upsell-order-bump-offer-for-woocommerce' ); ?></a>

						<a class="wps_wocuf_pro_funnel_links wps_upsell_bump_links" href="#"><?php esc_html_e( 'Report', 'upsell-order-bump-offer-for-woocommerce' ); ?></a>
						</div>
					</div>
				</div>
			<?php endforeach; ?>
...
<div class="wps_wocuf_pro_create_new_funnel">
	...
	<button type="button" id="wps-wocuf-open-funnel-template-modal"><?php esc_html_e( '+Create New Funnel', 'upsell-order-bump-offer-for-woocommerce' ); ?></button>
	<?php } ?>
</div>
...
<div class="wps-ubo-template-modal" id="wps-wocuf-funnel-template-modal" aria-hidden="true">
	...
	<div class="wps-ubo-template-dialog" role="dialog" aria-modal="true">
		<div class="wps-ubo-template-dialog__head">
			...
			<?php endif; ?>
			<button type="button" class="wps-ubo-template-close" id="wps-wocuf-funnel-template-close" aria-label="<?php esc_attr_e( 'Close', 'upsell-order-bump-offer-for-woocommerce' ); ?>">×</button>
		</div>
		<div class="wps-ubo-template-grid">
			<?php foreach ( $wps_wocuf_template_cards as $card ) : ?>
				...
				<div class="wps-ubo-template-card <?php echo $card_is_lock ? 'is-locked' : 'is-open'; ?>" data-template="<?php echo esc_attr( $card['slug'] ); ?>">
					...
					<?php if ( $card_is_lock ) : ?>
						<a href="https://wpswings.com/product/upsell-order-bump-offer-for-woocommerce-pro/?utm_source=order-bump-org&utm_medium=referral&utm_campaign=order-bump-pro" class="button button-primary wps-ubo-template-choose is-locked" target="_blank" rel="noopener noreferrer">
