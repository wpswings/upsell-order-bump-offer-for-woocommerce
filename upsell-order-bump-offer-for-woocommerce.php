...
add_action('wp_ajax_wps_refresh_bump_stats', 'wps_ajax_get_bump_stats_data');
function wps_ajax_get_bump_stats_data() {
...
/**
 * AJAX Handler for Post-Purchase Upsell Funnel Stats
 */
add_action('wp_ajax_wps_refresh_upsell_funnel_stats', 'wps_ajax_get_upsell_funnel_data');

function wps_ajax_get_upsell_funnel_data() {
        ...
        foreach ( $funnels_list as $id => $val ) {
            ...
            if ( $v > 0 || $a > 0 || $s > 0 || $rejects > 0 ) {
                $items[] = [
                    ...
                    // Updated data array: Views, Accepts, Success, Rejects, Triggered
