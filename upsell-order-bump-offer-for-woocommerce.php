...
add_action('admin_menu', function() {
    add_menu_page(
        ...
        'WPS Smart Popups',
        'manage_options',
        'wps-order-bump-smart-popup-settings',
        'wps_order_bump_smart_popup_admin_page',
 ...
 * ADMIN PAGE UI
 */
function wps_order_bump_smart_popup_admin_page() {

    if ( isset($_POST['wps_order_bump_save_popup_settings']) ) {

        update_option('wps_order_bump_popup_type', sanitize_text_field($_POST['wps_order_bump_popup_type']));
        update_option('wps_order_bump_popup_content', wp_kses_post($_POST['wps_order_bump_popup_content']));
        update_option('wps_order_bump_popup_delay', intval($_POST['wps_order_bump_popup_delay']));

        echo '<div class="updated"><p>Settings Saved</p></div>';
    }

    $popup_type    = get_option('wps_order_bump_popup_type', 'lightbox');
    $popup_content = get_option('wps_order_bump_popup_content', '<h2>Your Popup Content Here</h2><p>Write anything...</p>');
    $popup_delay   = get_option('wps_order_bump_popup_delay', 1);
    ?>

    <div class="wrap">
        <h1>WPS Smart Popup Settings</h1>
        <form method="post">

            <h3>Select Popup Type</h3>
            <select name="wps_order_bump_popup_type" style="width:300px;">
                <option value="lightbox" <?php selected($popup_type, 'lightbox'); ?>>Lightbox Popup</option>
                <option value="slide_in" <?php selected($popup_type, 'slide_in'); ?>>Slide-in Popup</option>
                <option value="fullscreen" <?php selected($popup_type, 'fullscreen'); ?>>Full Screen Welcome Mat</option>
            </select>

            <h3 style="margin-top:20px;">Popup Delay (in seconds)</h3>
            <input type="number" name="wps_order_bump_popup_delay" value="<?php echo esc_attr( $popup_delay ); ?>" style="width:120px;">
            <p class="description">Enter delay before popup appears.</p>

            <h3 style="margin-top:20px;">Popup Content</h3>
            <?php wp_editor($popup_content, 'wps_order_bump_popup_content'); ?>

            <p><input type="submit" name="wps_order_bump_save_popup_settings" class="button button-primary" value="Save Settings"></p>
        </form>
    </div>
...
/**
 * ------------------------------------------------------------
 * LOAD POPUP ONLY ON CART + CHECKOUT
 * ------------------------------------------------------------
 */
add_action('wp_footer', function() {
    ...
    $type    = get_option('wps_order_bump_popup_type', 'lightbox');
    $content = get_option('wps_order_bump_popup_content', '');
    $delay   = intval(get_option('wps_order_bump_popup_delay', 1)) * 1000;
    ?>

    <!-- POPUP WRAPPER -->
    <div id="wps-order-bump-popup" class="wps-order-bump-popup-<?php echo esc_attr($type); ?>">
        <div class="wps-order-bump-popup-inner">
            <?php echo wp_kses_post($content); ?>
            <button class="wps-order-bump-popup-close">&times;</button>
        </div>
    </div>

    <style>
        /* LIGHTBOX */
        .wps-order-bump-popup-lightbox {
            position: fixed; top:0; left:0; width:100%; height:100%;
            background: rgba(0,0,0,0.6);
            display:none; justify-content:center; align-items:center;
            z-index:999999;
        }
        .wps-order-bump-popup-lightbox .wps-order-bump-popup-inner {
            background:#fff; padding:25px; width:400px;
            border-radius:10px; position:relative;
        }

        /* SLIDE-IN */
        .wps-order-bump-popup-slide_in {
            position:fixed; bottom:-400px; right:20px; width:300px;
            background:#fff; padding:20px; border-radius:12px;
            box-shadow:0 0 20px rgba(0,0,0,0.2);
            transition:all .5s ease; z-index:999999;
        }
        .wps-order-bump-popup-slide_in.wps-show { bottom:20px; }

        /* FULLSCREEN */
        .wps-order-bump-popup-fullscreen {
            position:fixed; top:0; left:0; width:100%; height:100%;
            background:#fff; display:none; justify-content:center;
            align-items:center; z-index:999999;
        }
        .wps-order-bump-popup-fullscreen .wps-order-bump-popup-inner {
            max-width:700px; padding:40px; text-align:center;
        }

        .wps-order-bump-popup-close {
    ...
    <script>
        jQuery(document).ready(function($){
            ...
            const popup = $("#wps-order-bump-popup");
            const type  = popup.attr("class");

            function showWPSPopup() {
                if (type.includes("popup-lightbox")) {
                    popup.fadeIn();
                } 
                if (type.includes("popup-slide_in")) {
                    popup.addClass("wps-show");
                }
                if (type.includes("popup-fullscreen")) {
                    popup.fadeIn();
                }
            }

            // Delay from admin settings
            setTimeout(showWPSPopup, <?php echo $delay; ?>);

            $(".wps-order-bump-popup-close").on("click", function(){
                popup.fadeOut().removeClass("wps-show");
            });
